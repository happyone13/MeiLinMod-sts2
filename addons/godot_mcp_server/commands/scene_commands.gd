...
func setup(undo_manager: Node, editor_guards: Node, plugin: EditorPlugin) -> void:
...
func handle_set_instance_property(params: Dictionary, request_id: int = 0) -> Dictionary:
	...
	# Convert string values to the target property's native type (e.g. "Vector2(1,2)").
	if prop_value is String:
		var parsed = str_to_var(prop_value)
		if parsed != null:
			prop_value = parsed

	# Apply immediately so the scene is marked dirty and can be saved.
